<?php
namespace Jiny\Erp\Hr\Work\Http\Livewire;
    ...
    public function render()
    {
        ...
        if($user) {
            ...
            if($work) {
                ...
                // $dateTime = new \DateTime($work->time); // 시작 시간

                // $startHour = $dateTime->format('H'); // 시
                // $startMinute = $dateTime->format('i'); // 분

                // // 총 분 수 계산
                // $totalMinutes = ($startHour * 60) + $startMinute;
                // $this->start = $totalMinutes;
                $this->start = $work->time;
        ...
        return view("jinyerp-hr-work::user.workstatus.livewire_workstatus");
    }
